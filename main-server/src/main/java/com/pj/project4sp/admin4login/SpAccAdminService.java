...
public class SpAccAdminService {
    ...
    AjaxJson doLogin(String key, String password) {
        ...
        if (!admin.getPassword2().equals(md5Password)) {
            return AjaxJson.getError("用户名/密码错误");
        }
        ...
        map.put("tokenExpired", saTokenInfo.tokenTimeout);
        map.put("admin", admin);
    ...
    public AjaxJson douyinLogin(String username, String password) {
        SpAdmin spAdmin = spAdminMapper.getByName(username);
        if (spAdmin==null){
            spAdmin=spAdminMapper.getByPhone(username);
        }
        if (spAdmin==null){
        ...
        if (spAdmin.getStatus() == 2) {
            return AjaxJson.getError("此账号已被禁用，请联系管理员");
        }
        successLogin(spAdmin);
        StpUtil.logout(spAdmin.getId());
        StpUtil.login(spAdmin.getId());
        LoginBO loginBO = new LoginBO();
        BeanUtil.copyProperties(spAdmin, loginBO);
        // 组织返回参数
        SoMap map = new SoMap();
        SaTokenInfo saTokenInfo = StpUtil.getTokenInfo();
        map.put("userInfo", loginBO);
        map.put("token", saTokenInfo.tokenValue);
        return AjaxJson.getSuccessData(map);
    }
